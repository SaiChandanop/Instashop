...
function getProductFeed($paginationID)
{

	$query = "select * from products_description order by products_id desc";
	if (strlen($paginationID) > 0) 
	{
		$query = "select * from products_description where products_id < '$paginationID' order by products_id desc limit 30";
		if($paginationID == 0)
		{
			$query = "select * from products_description order by products_id desc limit 30";
		}
	}
	$result = mysql_query($query);
	...
	while ($row = mysql_fetch_assoc($result)) 
	{
		...
		// $responseObject["products_list_price"] = $row["products_list_price"];
		$responseObject["products_external_url"] = $row["products_external_url"];

		// $responseObject["products_price"] = "0.0000";
		// $responseObject["products_model"] = "";
		// $responseObject["products_date_added"] = "";		
...
function getProfileFeed($profileID)
{

	$query = "select * from products_description where products_description.products_instagram_id like '%_". $profileID ."' order by products_id desc";
	$result = mysql_query($query);
...
{
	$responseArray = array();
	$query = "select * from saved_products where instagram_id = '$instagramID' order by product_id desc";
...
else if ($_GET["requesting_seller_id"] == "FEED")
{
	$productsArray = getProductFeed();
...
//	$productsArray = getProductAttributes($productsArray);
	...
	$productsArray = getProfileFeed($_GET["requesting_seller_id"]);
...
else if (strlen($_GET["pagination_id"]) > 0)
{
	$productsArray = getProductFeed($_GET["pagination_id"]);
